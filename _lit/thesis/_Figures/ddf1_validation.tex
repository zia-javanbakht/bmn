...
ylabel =  {Thermal conductivity ($\sfrac{\text{W}}{\text{m}\cdot\text{K}}$), $k$},
ymax = 0.6,
ymin = 0,
%ytick pos=left,
...
% Experimental (mean)
% horizontal line
%\addplot[samples=10, thick,  domain=17:22] {8190};
...
\def\B{ (\kfkm-1)/(\kfkm+\A)  }
\def\pac{0.52}
\def\si{1+(x/100)*(1-\pac)/(\pac*\pac)}
\def\equ{ \km*(1.0+\A*\B*(x/100))/(1.0-\B*\si*(x/100))}

\def\eff{0.2} % efficiency parameter in RoM (in percentage)

% Experimental (samples)
\addplot
[mark=*, densely dashed,%only marks,
mark options={black,fill=gray},
mark size=3pt,
error bars/.cd,
y dir=both,
y explicit,
...
% ,y error index=2
 ...
 {./_data_ddf1/volfrac_cond};

 
 
 % RoM
 \addplot[samples=20, thick, dotted, smooth, domain=0:1] { (\eff)*(x/100)*\kf +(1-(x/100))*\km  };
 
 % IRoM
% \addplot[samples=20, thick, smooth, domain=0:10] {1/((x/100)/\kf +((1-x)/100)/\km)};
 
 
 % Halpin-Tsai
 \addplot[samples=200, thick, smooth, domain=0:10] {0.214*(1+8.374358*(x/100))/(1-2.77328*(x/100))};
 
 
 
 \begin{pgfonlayer}{axis foreground}
	% annotation
	\draw[thin,arrows={Stealth[length=2.5mm,fill=white]-}] (axis cs: 0.1,0.22) to  [out=65,in=225] +(6,15) node[inner sep =1, anchor=south west,text width=2cm] {\small Pure matrix};
 \end{pgfonlayer}
...
% \addplot[samples=200, thick, smooth, domain=1:10] {0.214*(1+8.38*(x/100)*(2980-0.214)/(2980+8.38*0.214))/(1-(1+(x/100)*(1-0.52)/(0.52*0.52))*(2980-0.214)/(2980+8.38*0.214))};
 
% \addplot[samples=500, thick, smooth, domain=0:10] {(1+\A*(\kfkm-1)/(\kfkm+\A)*(x/100)*\km)/(1-((1+(x/100)*(1-\pac)/(\pac^2))*(\kfkm-1)/(\kfkm+\A)))};
 
 
%% NOTE
%%	The updating and w/o updating values are the same but the not_updating file is not updated.
%
%% Computational (w/o updating)
%% \addplot
...
%% 	y dir=both,
%% 	y explicit,
%% 	error bar style={gray, line width=1pt},
%% 	error mark options={rotate=90, gray, mark size=3pt, line width=1.pt}
%% ] table [
%%  x index=0, 
%%  y index=1, 
%%  y error index=2
%%  ] 
%%  {./_data_ch8/E_strength_not_updating.tex};
% 
%
%% % Computational (w/t updating)
% \addplot
...
%  ,y error index=2
%  ] 
%  {./_data_ch8/E_strength_updating.tex};
%  
%
%	 
%
%% RoM
%\addplot[name path=rom, dashed,  thick,
...
% 	 
% % En-RoM 
% 	\addplot[name path=enrom,no marks, very thick,  dashdotted] table [x index=0, y index=1] 
% 	{./_data_ch8/E_enrom.tex};
% 
%
%
%	% CLassic IRoM-RoM
%	\addplot[opacity=0.3, gray!50] fill between [of=rom and irom]; 
%	
%	% RoM-En-RoM
%	\addplot[opacity=1, gray!50] fill between [of=rom and enrom]; 
...
% Legends
  ...
  \addlegendentry{Computational (mean)};
  \addlegendentry{RoM ($\varkappa_\text{o}=\eff$)};
  \addlegendentry{LN ($A=8.38$, $\zeta_\text{f,max}=0.52$)};
  \addlegendentry{Lewis-Nielson ($A=8.38$)};
  \addlegendentry{Computational ($+/-$)};
%  \addlegendentry Computational ($+$)};
  \addlegendentry{IRoM};
  \addlegendentry{En-RoM};
  \addlegendentry{Classic IRoM-RoM};
  \addlegendentry{Extension by En-RoM};
\end{axis}
