...
ytick distance		= 20,
grid				= major,
xlabel				= {Fibre area ($\upmu \text{m}^2$)},
ylabel				= {Frequency},
scaled x ticks 		= false,
xticklabel style	= {
	/pgf/number format/fixed, xshift=-18,
%	/pgf/number format/1000 sep =,
%	rotate							= 90,
%	anchor							= west,
%	/pgf/number format/fixed,
	},
legend style 		= {draw=none, at = {(0.45,0.9)}, anchor=west, legend cell align=center, text width=3.1cm, align=center,fill=none}	
],
\addplot[%
	hist={bins=23,data=x,},%
	bar width			= 2pt,
...
file {./_data_ch8/app_area_dist_data.tex};
\addlegendentry{\small Apparent area}
\end{axis}
% True area fit
\begin{axis}[
width=8cm,
...
axis y line			= none,
inner sep			= 0pt,
legend style 		= {draw=none, at = {(0.45,0.7)}, anchor=west, legend cell align=center, text width=2.5cm, align=center,fill=none},
%yshift				= -13
],
\addplot[% CSA distribution Log-normal (mean=7.55, SD=0.52)
	domain			= 0:10000,
	samples			= 500,
	no marks,
	dashed,
...
{320000*exp(-((ln(x)- 7.55)^2)/(2*0.52^2))/(x*0.52*sqrt(2*pi)) };
\addlegendentry{\small True area fit}
\end{axis}
...
axis y line			= none,
inner sep			= 0pt,
legend style 		= {draw=none,at = {(0.45,0.8)}, anchor=west, legend cell align=center, text width=2.5cm, align=center,fill=none}
%yshift				= -13
],
\addplot[% CSA distribution Log-normal (mean=7.55, SD=0.52)
	domain			= 0:10000,
...
{380000*exp(-((ln(x)- 7.90)^2)/(2*0.52^2))/(x*0.52*sqrt(2*pi)) };
%{exp(-((ln(x)- 65.6)^2)/(2*2.65^2))/(x*2.65*sqrt(2*pi)) };
\addlegendentry{\small Apparent area fit}
\draw[fill=white] (2.75cm,2.76cm) rectangle +(3.4cm,1.5cm);
\end{axis}
