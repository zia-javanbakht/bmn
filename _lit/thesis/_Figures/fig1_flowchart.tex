...
\node[fterm                                                                                       ]   (s)     {Start};
	\node[fio, below = \y of s, text width=] (inp) {Get the input data};
	\node[fproc, below = \y of inp, inner ysep=2     , inner xsep=1 , text width=3cm  ]   (sub1)  {Calculate the\\ required fibre length};
	\node[fjunc , below = \y of sub1 ]   (j1)    {};
	\node[fdeci , below = \y of j1, inner sep=0.1     , minimum size=1cm                        ]   (dec1)    {Non-zero\\remaining\\length?};

	% FALSE
	\node[fproc, below = \y of dec1, inner ysep=2     , inner xsep=0.01  , text width=3cm ]   (sub2)  {Export the fibre\\structure to the data file};
	\node[fproc, below = \y of sub2, inner ysep=2     , inner xsep=0.1  , text width=3cm ]   (sub3)  {Create the FE model\\using \fc{MENTAT}};
	\node[fproc, below = \y of sub3, inner ysep=2     , inner xsep=0.1  , text width=3.cm ]   (sub4)  {Submit the job to \fc{MARC}};
	\node[fproc, below = \y of sub4, inner ysep=2     , inner xsep=0.1  , text width=3.cm ]   (sub5)  {\fc{USDATA()} reads the\\fibre structure data file};
	\node[fproc, below = \y of sub5, inner ysep=2     , inner xsep=0.1 , text width=3.cm ]   (sub6)  {Solve the\\ system of equations};
	\node[fproc, below = \y of sub6, inner ysep=2     , inner xsep=0.1  , text width=3.cm ]   (sub7)  {Increase the increment};
		\node[fjunc , below = \y of sub7 ]   (j2)    {};

	
	% CONVERGENCE branch
	\node[fdeci, right = \x of sub6, inner sep=0.1     , minimum size=1cm                        ]   (dec2)    {Convergence?};
	\node[fdeci, right = \x of dec2, inner sep=0.1     , minimum size=1cm                        ]   (dec3)    {Fibre/matrix\\failure?};
	\node[fproc, right = \x of dec3, inner ysep=2     , inner xsep=0.1  , text width=3.cm ]   (sub8)  {Remove the failed\\fibre/matrix elements};
	\node[fproc, below = \y of sub8, inner ysep=2     , inner xsep=0.1  , text width=3.cm ]   (sub9)  {Update the mesh};
	\node[fdeci, below = \y of sub9, inner sep=0.1     , minimum size=1cm                        ]   (dec4)    {Last\\increment?};
	\node at (dec3 |- dec4) [fproc, inner ysep=2, anchor=center     , inner xsep=0.1  , text width=2.5cm ]   (sub10)  {Update the\\ connectivity of fibres};
	\node at (dec2 |- dec4)[fproc,very thick,inner ysep=2     , inner xsep=0.1  , text width=2.5cm ]   (sub11)  {Update the\\ strength of fibres};

	% postprocess
	\node[fproc, below = \y of dec4, inner ysep=2     , inner xsep=0.1  , text width=2.5cm ]   (sub12)  {Post-processing\\in \fc{MENTAT}};
	\node at (s |- sub12) [fterm]   (e)     {End};
	\node at (sub10 |- sub12) [fproc, inner ysep=2     , inner xsep=0.1  , text width=2.5cm ]   (sub13)  {Stress-strain\\ calculations};
	\node at (sub11 |- sub12) [fproc, inner ysep=2     , inner xsep=0.1  , text width=2.5cm ]   (sub14)  {Export the results\\ to \fc{EXCEL}};
	
	% TRUE
	\node at (dec2 |- dec1) [fproc, inner ysep=2     , inner xsep=0.1  , text width=2.5cm ]   (sub1-1)  {Create a fibre with pseudo-random\\coordinates};
	\node at (dec3 |- sub1-1)[fdeci ,  inner sep=0.1     , minimum size=1cm ]   (dec1-1)    {Fibre outside\\matrix?};
	\node at (dec1-1 |- j1)[fproc, inner ysep=2     , inner xsep=0.1  , text width=2.5cm ]   (sub1-3)  {Group the\\(segmented) fibre(s)};
	\node at (dec2 |- j1)[fproc, inner ysep=2     , inner xsep=0.1  , text width=2.5cm ]   (sub1-4)  {Assign a CSA to the group};
	\node at (dec1-1 |- sub2) [fproc, inner ysep=2     , inner xsep=0.1  , text width=2.5cm ]   (sub1-2)  {Cut \& move the\\remaining segment};		

\draw[fconc]  (s.south)     --   (inp.north);
\draw[fconc]  (inp.south)     --   (sub1.north);
\draw[sconc]  (sub1.south)     --   (j1.north);
\draw[fconc]  (j1.south)     --   (dec1.north);
\draw[fconc]  (dec1.south)     --   (sub2.north)  node [midway,right] {F};
\draw[fconc]  (sub2.south)     --   (sub3.north);
\draw[fconc]  (sub3.south)     --   (sub4.north);
\draw[fconc]  (sub4.south)     --   (sub5.north);
\draw[fconc]  (sub5.south)     --   (sub6.north);
\draw[fconc]  (sub6.east)     --   (dec2.west);
\draw[fconc]  (dec2.east)     --   (dec3.west) node [midway,above] {T};
