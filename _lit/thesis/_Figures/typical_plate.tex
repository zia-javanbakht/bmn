...
% height
\path[draw,] (-0.2,\the\ply+.2,-\the\plt/2) -- ++ (0,0,+\the\plt/2) node [midway, left] {$\textstyle \sfrac{h}{2}$};;
\path[draw] (-0.2,\the\ply+.2,+\the\plt/2) -- ++ (0,0,-\the\plt/2) node [midway, left] {$\textstyle \sfrac{h}{2}$};


% Length
\path[draw] (-0.25,\the\ply,\the\plt) -- ++ (\the\plx,0,0) node [midway, below] {$l_1$};
\path[draw] (\the\plx,-0.25,\the\plt+0.1) -- ++ (0,\the\ply,0) node [midway, below] {$l_2$};
...
% Annotation
\begin{scope}[thin,arrows={-Stealth[length=2.5mm,fill=white]}]%-{Kite[fill=white, length=\the\annottext]}]
  \path[draw] (\the\plx+1,1,-1.5) -- (\the\plx,1.5,0) node [pos=0.1, anchor=south west] {Middle-plane};
% \path[draw] (2,-1,1) -- (2,0,0) node [pos=0, anchor=north] {Middle-plane};
\end{scope}
